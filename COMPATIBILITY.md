...
The current compatibility range includes MDT 6.2.9. The 2026-08-28 source review pins upstream tag `6.2.9` at commit `a49bd78f843bb89d2cea3daefbc1cf7aed809c31`. Relative to the previously reviewed 6.2.x source, the current public `Modules/API.lua` contract is unchanged, while `Modules/Presets.lua` still uses the `db.currentDungeonIdx`, `db.currentPreset` and `db.presets` route-selection layout consumed by the adapter. The 6.2.9 release changelog is limited to dungeon enemy-force corrections, so no MDTPullMarker runtime adapter change is required.
...
A version marked `source-verified` means its adapter/source contract was explicitly reviewed and is present in the runtime's explicit verified-version allowlist. MDT 6.2.9 remains inside the accepted compatibility range but is deliberately reported as `compatible-range` by the current rc62 runtime; the 2026-08-28 source review does not silently change that runtime classification. Live-client validation remains a separate release gate, and source review alone does not prove in-game behavior.
