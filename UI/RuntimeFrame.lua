...
function RuntimeFrame:Refresh()
  ...
  elseif ownership.owner and ownership.isOwner == false then frame.macros:SetText("|cff8fa3b8Passive - "..tostring(ownership.owner).." is marker owner|r")
  ...
  elseif boundMode and routeMacros.current == true then frame.macros:SetText("|cffffad33Route macros parked until this dungeon/client is active|r")
  ...
  if boundMode then
    ...
    if submission and submission.allRequired then flowText = "Markers submitted • next marked pull loads after combat"
    elseif submission and submission.batch1 and submission.batch2Required then flowText = "MDTPM1 submitted  ->  wait ~4 sec  ->  MDTPM2"
  ...
  elseif ownership.owner and ownership.isOwner == false then frame.help:SetText("This client stays idle to prevent duplicate party marking.")
  ...
  elseif executor.pendingPullAdvance then frame.help:SetText("Markers submitted - waiting for combat to end before loading the next MDT pull.") else frame.help:SetText(flowText) end
  return runtime
end
