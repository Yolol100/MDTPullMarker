...
function MarkerMacro.ParseBulkToken(token)
  ...
  if legacyPull then return tonumber(legacyPull), tonumber(legacyBatch), legacyAssignment, tonumber(legacyCount), nil, "legacy-batch" end
...
function MarkerMacro.BuildBulkToken(identities)
  ...
  for _, identity in ipairs(identities) do
    ...
    if assignmentID == "" or assignmentID:find("[^%w:_%-]") or not marker or marker < 1 or marker > 8 or targetName == "" then return nil end
    parts[#parts + 1] = table.concat({ assignmentID, tostring(marker), targetName, executionMethod, "set-if-unmarked" }, "\31")
  end
...
function MarkerMacro.BuildBulkBody(identities)
  ...
  for _, identity in ipairs(identities) do
    ...
    if tostring(identity.executionMethod or "exact-name") ~= "exact-name" then return nil, "same-name-automatic-targeting-unavailable" end
...
local function isRecognizedBulkMacroBody(body)
  ...
  if format == "compact" or format == "signed" then
    ...
    while lineIndex < #lines do
      ...
      if line == "/cleartarget" then
        ...
        if not markerLine or (not markerLine:match("^/tm ~[1-8]$") and not markerLine:match("^/tm %[harm%] ~[1-8]$") and not markerLine:match("^/tm %[harm,nodead%] ~[1-8]$")) then return false end
        assignmentCount = assignmentCount + 1
        lineIndex = lineIndex + 3
      elseif line:match("^/tm %[@mouseover,harm,nodead%] ~[1-8]$") or line:match("^/tm %[@target,harm,nodead%] ~[1-8]$") then
        assignmentCount = assignmentCount + 1
  ...
  for index, line in ipairs(lines) do
    ...
    elseif line:match("^/tm [1-8]$") or line:match("^/tm ~[1-8]$") or line:match("^/tm %[harm,nodead%] [1-8]$") or line:match("^/tm %[harm,nodead%] ~[1-8]$") or line:match("^/tm %[@target,harm,nodead%] [1-8]$") or line:match("^/tm %[@mouseover,harm,nodead%] [1-8]$") then
...
local LEGACY_CONDITIONS = {
  { marker = 1, modifier = "ctrlshiftalt" }, { marker = 2, modifier = "ctrlalt" },
  { marker = 3, modifier = "shiftalt" }, { marker = 4, modifier = "ctrlshift" },
  { marker = 5, modifier = "alt" }, { marker = 6, modifier = "ctrl" },
  { marker = 7, modifier = "shift" }, { marker = 8, modifier = nil },
}
...
function MarkerMacro.IsRecognizedBody(body)
  ...
  if body:match("^/stopmacro %[combat%]\n/stopmacro %[@mouseover,noexists%]%[@mouseover,noharm%]%[@mouseover,dead%]\n/mpm markarm [Ee]%d+:[Cc]%d+\n/tm %[@mouseover,harm,nodead%] ~?%d$") then return true end
  if body:match("^/stopmacro %[combat%]\n/cleartarget\n/targetexact [^\r\n]+\n/stopmacro %[noexists%]%[noharm%]%[dead%]\n/mpm markarm [Ee]%d+:[Cc]%d+\n/tm ~?%d$") then return true end
  if body:match("^/stopmacro %[combat%]\n/stopmacro %[@mouseover,noexists%]%[@mouseover,noharm%]%[@mouseover,dead%]\n/tm %[@mouseover,harm,nodead%] ~?%d\n/mpm markdone [Ee]%d+:[Cc]%d+$") then return true end
  if body:match("^/stopmacro %[combat%]\n/cleartarget\n/targetexact [^\r\n]+\n/stopmacro %[noexists%]%[noharm%]%[dead%]\n/tm ~?%d\n/mpm markdone [Ee]%d+:[Cc]%d+$") then return true end
