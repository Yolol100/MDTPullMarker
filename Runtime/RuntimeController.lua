...
local function copy(value) return DataUtils.DeepCopy(value) end
local function refreshMarkerInstruction(reason) return Addon.MarkerExecutor:OnInstructionChanged(reason or "runtime-instruction-changed") end
local function refreshDeathTracker(reason, force) return Addon.PullDeathTracker:RefreshContext(reason or "runtime-pull-changed", force == true) end
local function currentPull() local plan = state.plan return plan and plan.pulls and plan.pulls[state.currentPullPosition] or nil end
local function assignmentsForPull(pull) return type(pull) == "table" and type(pull.assignments) == "table" and pull.assignments or {} end
...
local function buildPlanSignature(plan)
  ...
  for _, pull in ipairs((plan and plan.pulls) or {}) do
    ...
    if death then
      ...
      table.sort(ids)
      for _, npcID in ipairs(ids) do parts[#parts + 1] = ("n=%d:%d"):format(npcID, tonumber(death.expectedByNPC[npcID]) or 0) end
    end
    for _, assignment in ipairs(assignmentsForPull(pull)) do
      parts[#parts + 1] = table.concat({
        tostring(assignment.id or "no-id"), tostring(assignment.marker or 0), tostring(assignment.requestedMarker or assignment.marker or 0),
        assignment.markerRemapped == true and "1" or "0", assignment.automaticTargeting == false and "manual" or "auto",
        tostring(assignment.priority or 0), tostring(assignment.batchIndex or 0), tostring(assignment.batchPosition or 0),
        tostring(assignment.executionMethod or ""), assignment.useSetUnmarked == true and "1" or "0",
        tostring(assignment.duplicateGroupName or ""), tostring(assignment.duplicateNameOrdinal or 1), tostring(assignment.npcID or 0),
        tostring(assignment.duplicateTotal or 1), tostring(assignment.routeNameTotal or 1), tostring(assignment.dungeonNameTotal or 0),
        tostring(assignment.targetNameTotal or assignment.routeNameTotal or 1), tostring(assignment.adjacentPullNameTotal or 0),
        tostring(assignment.routeTargetNameTotal or 0), tostring(assignment.otherPullNameTotal or 0), tostring(assignment.targetNameScope or ""),
        tostring(assignment.targetName or assignment.name or ""), assignment.enabled == false and "0" or "1",
      }, "\31")
    end
  end
...
local function markerPoolForPull(pull)
  ...
  for _, assignment in ipairs(assignmentsForPull(pull)) do
    ...
    if marker and marker >= 1 and marker <= 8 then
      ...
      if assignment.id and confirmed[tostring(assignment.id)] then status = "confirmed"
      elseif assignment.id and submitted[tostring(assignment.id)] then status = "submitted" end
      result[marker] = { marker = marker, status = status, assignmentID = assignment.id, targetName = assignment.targetName or assignment.name }
    end
  end
  return result
end
local function firstUnconfirmedPosition(pull)
  ...
  local confirmed = confirmedForPull(pull)
  for position, assignment in ipairs(assignments) do if not assignment.id or not confirmed[assignment.id] then return position end end
  return math.max(1, #assignments)
end
local function resetCursor()
  state.currentInstructionPosition = 1
  local pull = currentPull()
  if pull and #assignmentsForPull(pull) > 0 then state.currentInstructionPosition = firstUnconfirmedPosition(pull) end
end
local function clampPullPosition()
  local count = state.plan and #(state.plan.pulls or {}) or 0
  if count == 0 then state.currentPullPosition = 1 return end
...
local function updateStatus()
  if not state.plan or state.plan.status == "blocked" then state.status = "blocked" return end
  local pull = currentPull()
  if not pull then state.status = "empty" return end
  if pull.status == "manual-required" and not state.completedPulls[pull.index] then state.status = "manual-required" return end
  state.status = state.completedPulls[pull.index] and "completed" or "ready"
end
local function firstFindingCode(findings)
  if type(findings) == "table" and type(findings[1]) == "table" then return tostring(findings[1].code or findings[1].severity or "marker-plan-unavailable") end
...
function Controller:Refresh(reason, useCurrentSnapshot)
  ...
  if useCurrentSnapshot then snapshot = Addon.MDT:GetSnapshot()
  else
    snapshot, refreshError = Addon.MDT:Refresh(reason or "runtime-refresh")
    if not snapshot then
      state.plan, state.routeMacroPlan, state.findings = nil, nil, {}
  ...
  if not plan then
    state.plan, state.routeMacroPlan = nil, nil
  ...
  if preferredPull then
    for position, pull in ipairs(plan.pulls or {}) do if pull.index == preferredPull then state.currentPullPosition = position break end end
  end
  clampPullPosition()
  resetCursor()
  state.lastError = nil
  ...
  if Addon.MarkerExecutor and type(Addon.MarkerExecutor.OnRuntimeValidated) == "function" then
    Addon.MarkerExecutor:OnRuntimeValidated(refreshReason)
  else
...
function Controller:Initialize() state.status = "initializing" return self:Refresh("runtime-initialize", true) end
function Controller:ResetProgress(reason)
  state.completedPulls, state.skippedPulls, state.engagedPulls = {}, {}, {}
  state.confirmedAssignments, state.submittedAssignments = {}, {}
  state.currentPullPosition, state.currentInstructionPosition = 1, 1
  state.lastMarkerResult = nil
  clampPullPosition(); resetCursor(); updateStatus()
...
function Controller:GetPullByIndex(pullIndex)
  ...
  for _, pull in ipairs(state.plan.pulls or {}) do if tonumber(pull.index) == pullIndex then return copy(pull) end end
end
function Controller:GetPullPositionByIndex(pullIndex)
  pullIndex = DataUtils.PositiveInteger(pullIndex)
  if not pullIndex or not state.plan then return nil end
  for position, pull in ipairs(state.plan.pulls or {}) do if tonumber(pull.index) == pullIndex then return position end end
end
function Controller:GetRouteMacroPlan()
  return Addon.RouteMacroPlan and Addon.RouteMacroPlan.Copy and Addon.RouteMacroPlan.Copy(state.routeMacroPlan) or copy(state.routeMacroPlan)
end
function Controller:ResolveRouteMacroToken(token)
  if not Addon.RouteMacroPlan or type(Addon.RouteMacroPlan.ResolveToken) ~= "function" then return nil, "route-macro-planner-unavailable" end
...
function Controller:GetRouteMacroDescriptor(pullIndex, batchIndex)
  if not Addon.RouteMacroPlan or type(Addon.RouteMacroPlan.GetDescriptor) ~= "function" then return nil, "route-macro-planner-unavailable" end
  ...
  if reason == "route-macro" then
    ...
    if previousPosition and position > previousPosition then
      for skippedPosition = previousPosition, position - 1 do
        ...
        if skipped and not state.completedPulls[skipped.index] and not state.engagedPulls[skipped.index] then state.skippedPulls[skipped.index] = true end
      end
    end
  end
  state.currentPullPosition = position
  resetCursor(); state.lastMarkerResult = nil; updateStatus()
...
function Controller:SetPullByIndex(pullIndex, reason)
  ...
  for position, pull in ipairs(state.plan.pulls or {}) do if pull.index == pullIndex then return self:SetPullByPosition(position, reason) end end
...
function Controller:CompletePullByIndex(pullIndex, options)
  ...
  state.completedPulls[pullIndex], state.skippedPulls[pullIndex], state.engagedPulls[pullIndex] = true, nil, true
  if Addon.PullDeathTracker and type(Addon.PullDeathTracker.RetirePull) == "function" then Addon.PullDeathTracker:RetirePull(pullIndex, "pull-completed") end
  updateStatus()
  if options.advance == true and position == state.currentPullPosition and position < #(state.plan.pulls or {}) then return self:SetPullByPosition(position + 1, "pull-completed") end
  if options.silent ~= true then refreshMarkerInstruction("pull-completed") end
  return self:GetState()
end
function Controller:CompleteCurrentPull(advance)
  local pull = currentPull(); if not pull then return nil, "pull-unavailable" end
  return self:CompletePullByIndex(pull.index, { advance = advance == true })
end
function Controller:ReopenCurrentPull()
  local pull = currentPull(); if not pull then return nil, "pull-unavailable" end
  state.completedPulls[pull.index], state.skippedPulls[pull.index] = nil, nil
  updateStatus(); refreshDeathTracker("pull-reopened", true); refreshMarkerInstruction("pull-reopened")
  return self:GetState()
end
...
function Controller:OnMarkerApplied(markerIndex, assignmentID)
  ...
  if assignmentID ~= nil and tostring(assignmentID) ~= tostring(assignment.id) then
    state.lastMarkerResult = { status = "unexpected-assignment", expectedAssignmentID = assignment.id, observedAssignmentID = tostring(assignmentID), marker = tonumber(markerIndex) }
    return nil, "assignment-mismatch"
  end
  markerIndex = tonumber(markerIndex)
  if markerIndex ~= tonumber(assignment.marker) then
    state.lastMarkerResult = { status = "unexpected", expected = tonumber(assignment.marker), observed = markerIndex, assignmentID = assignment.id }
    return nil, "unexpected-marker"
  end
  local confirmed = confirmedForPull(pull)
  if assignment.id then confirmed[assignment.id] = true end
  state.lastMarkerResult = { status = "confirmed", marker = markerIndex, assignmentID = assignment.id, pullIndex = pull.index }
  ...
  if not global or global.autoAdvanceInstructions ~= false then
    for nextPosition = position + 1, #assignments do
      ...
      if not nextAssignment.id or not confirmed[nextAssignment.id] then state.currentInstructionPosition = nextPosition return self:GetState(), "instruction-confirmed" end
    end
  end
  local allConfirmed = #assignments > 0
  for _, candidate in ipairs(assignments) do if not candidate.id or not confirmed[candidate.id] then allConfirmed = false break end end
...
function Controller:OnBatchSubmitted(items)
  ...
  local valid = {}
  for _, assignment in ipairs(assignmentsForPull(pull)) do if assignment.id then valid[tostring(assignment.id)] = tonumber(assignment.marker) end end
  local submitted = submittedForPull(pull)
...
local function pullIndexList(source)
  local result = {}
  for pullIndex, enabled in pairs(source or {}) do pullIndex = tonumber(pullIndex) if enabled == true and pullIndex then result[#result + 1] = pullIndex end end
...
function Controller:GetState()
  ...
  for _, candidate in ipairs(assignments) do if candidate.id and confirmed[candidate.id] then confirmedCount = confirmedCount + 1 end end
  return {
    status = state.status, planStatus = state.plan and state.plan.status or "unavailable", routeKey = state.plan and state.plan.routeKey,
    dungeonName = state.plan and state.plan.dungeonName, presetName = state.plan and state.plan.presetName,
    routeFingerprint = state.plan and state.plan.routeFingerprint, observedFingerprint = state.plan and state.plan.observedFingerprint,
    currentPullPosition = state.currentPullPosition, currentPullIndex = pull and pull.index,
    pullCount = state.plan and #(state.plan.pulls or {}) or 0, routePullCount = state.plan and state.plan.summary and state.plan.summary.routePulls or 0,
    ...
    routeMacroPlanError = state.lastRouteMacroError, assignmentCount = #assignments,
    instructionPosition = assignment and state.currentInstructionPosition or 0, instructionCount = #assignments, confirmedCount = confirmedCount,
    allAssignmentsConfirmed = #assignments > 0 and confirmedCount == #assignments, assignment = copy(assignment),
    assignmentConfirmed = assignment and assignment.id and confirmed[assignment.id] == true or false,
    completed = pull and state.completedPulls[pull.index] == true or false, skipped = pull and state.skippedPulls[pull.index] == true or false,
    completedPulls = pullIndexList(state.completedPulls), skippedPulls = pullIndexList(state.skippedPulls), engagedPulls = pullIndexList(state.engagedPulls),
    pullStatus = pull and pull.status or nil, automaticTargeting = pull and pull.automaticTargeting ~= false or false,
    markerPool = copy(markerPoolForPull(pull)), deathTracking = Addon.PullDeathTracker:GetState(), findings = copy(state.findings),
    lastMarkerResult = copy(state.lastMarkerResult), lastError = state.lastError,
  }
end
