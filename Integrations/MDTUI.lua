...
local state = {
  ...
  mode = "none",
  pendingSection = nil,
  legacyFrameCallbackRegistered = false,
}
...
local function setPending(message)
  state.lastError = tostring(message or "pending")
  return nil, state.lastError
end

local function getPublicAPI()
...
local function legacySectionExists(legacy, key)
  if type(legacy.GetNavigationSection) == "function" then
    local ok, section = pcall(legacy.GetNavigationSection, legacy, key)
    if ok then return section end
  end
  local lookup = legacy.navigationSectionLookup
  return type(lookup) == "table" and lookup[key] or nil
end

local function makeLegacyPlugin(legacy)
  local plugin = { legacy = legacy }

  function plugin:RegisterNavigationSection(section)
    local existing = legacySectionExists(legacy, section.key)
    if existing then return existing end
    return legacy:RegisterNavigationSection(section)
  end

  function plugin:GetNavigationSectionContentFrame(key)
    local frame = legacy.main_frame
    local frames = frame and frame.sectionContentFrames
    return type(frames) == "table" and frames[key] or nil
  end

  function plugin:SetCurrentSection(key)
    return legacy:SetCurrentSection(key)
  end
...
local function applyPendingSection()
  local sectionKey = state.pendingSection
  ...
  if ok then
    ...
    setError("set-section-failed:"..tostring(sectionError))
  end
end

function Integration:BuildPanel()
  if state.panel then
    if Addon.ConfigurationUI then Addon.ConfigurationUI:Refresh() end
    return state.panel
  end
  local plugin = state.plugin
  if type(plugin) ~= "table" or type(plugin.GetNavigationSectionContentFrame) ~= "function" then
    return setError("content-frame-api-unavailable")
  end
  ...
  if not Addon.ConfigurationUI or type(Addon.ConfigurationUI.CreateEmbedded) ~= "function" then
    return setError("configuration-ui-unavailable")
  end
...
function Integration:Attach(plugin, mode)
  ...
  if type(plugin.RegisterNavigationSection) ~= "function"
    or type(plugin.GetNavigationSectionContentFrame) ~= "function"
    or type(plugin.SetCurrentSection) ~= "function" then
    return setError("required-plugin-method-missing")
  end

  state.plugin = plugin
  state.mode = mode or state.mode or "unknown"
  local ok, result = pcall(plugin.RegisterNavigationSection, plugin, {
    key = SECTION_KEY,
    name = "Pull Markers",
    tooltip = "Route-bound target markers for the active MDT route",
...
local function registerPublic(api)
  ...
  state.mode = "public-plugin"
  state.lastError = nil
  return true
end

local function registerLegacy(legacy)
  if type(legacy.RegisterNavigationSection) ~= "function" or type(legacy.SetCurrentSection) ~= "function" then
    return nil, "legacy-navigation-api-unavailable"
  end

  local existing = legacySectionExists(legacy, SECTION_KEY)
  if legacy.main_frame and not existing then
    -- MDT 6.1.x builds the sidebar and section frames only once. Registering a new
    -- section after that point would hide the map without creating a usable page.
    state.registered = true
    state.mode = "legacy-standalone-fallback"
    state.lastError = "legacy-ui-already-initialized"
  ...
  if not state.legacyFrameCallbackRegistered and type(legacy.RunAfterFramesInitialized) == "function" then
    ...
    local ok, callbackError = pcall(legacy.RunAfterFramesInitialized, legacy, function()
      Integration:BuildPanel()
      applyPendingSection()
    end)
    if not ok then
      state.legacyFrameCallbackRegistered = false
      if Addon.Log then Addon.Log("WARN", "MDT frame-ready callback failed: "..tostring(callbackError), false) end
    end
  end
...
function Integration:Register()
  ...
  if state.registered and state.mode == "legacy-standalone-fallback" then
    return nil, state.lastError or "legacy-ui-already-initialized"
  end
...
local function showMDT()
  local api = getPublicAPI()
  if type(api) == "table" and type(api.ShowInterface) == "function" then
    local ok, showError = pcall(api.ShowInterface, api, true)
    if not ok then return nil, "show-mdt-failed:"..tostring(showError) end
    return true
  end

  local legacy = getLegacyMDT()
  if type(legacy) == "table" and type(legacy.ShowInterface) == "function" then
    local ok, showError = pcall(legacy.ShowInterface, legacy, true)
    if not ok then return nil, "show-mdt-failed:"..tostring(showError) end
    return true
  end
...
local function requestSection(sectionKey)
  state.pendingSection = sectionKey
  if state.attached then applyPendingSection() end
end
...
function Integration:OpenSection()
  ...
  local shown, showError = showMDT()
  if not shown then return setError(showError) end
  applyPendingSection()
  return true
end
...
local function setNamedSection(sectionKey)
  ...
  if state.attached and type(plugin) == "table" and type(plugin.SetCurrentSection) == "function" then
    local ok, sectionError = pcall(plugin.SetCurrentSection, plugin, sectionKey)
    if not ok then return nil, "set-section-failed:"..tostring(sectionError) end
    state.pendingSection = nil
    return true
  end
  ...
  if type(legacy) == "table" and type(legacy.SetCurrentSection) == "function" then
    local section = legacySectionExists(legacy, sectionKey)
    if not section then return nil, "mdt-section-unavailable:"..tostring(sectionKey) end
    local ok, sectionError = pcall(legacy.SetCurrentSection, legacy, sectionKey)
    if not ok then return nil, "set-section-failed:"..tostring(sectionError) end
    state.pendingSection = nil
    return true
  end
...
function Integration:OpenNamedSection(sectionKey)
  ...
  if sectionKey == "" then return setError("section-key-missing") end
  requestSection(sectionKey)
  local registered, registrationError = self:Register()
  if not registered and registrationError ~= "legacy-ui-already-initialized" then
    state.pendingSection = nil
    return nil, registrationError
  end
  ...
  local shown, showError = showMDT()
  if not shown then return setError(showError) end
  local selected, sectionError = setNamedSection(sectionKey)
  if not selected then return setError(sectionError) end
  return true
end

function Integration:OpenMap()
  local opened, openError = self:OpenNamedSection("maps")
  if opened and Addon.MDT then Addon.MDT:Refresh("open-mdt-map") end
  return opened, openError
end

function Integration:OpenFocusMarkers()
  return self:OpenNamedSection("marks")
end
